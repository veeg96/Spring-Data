		...
		try (Scanner scanner = new Scanner(System.in)) {
			do {
				...
				case 5:
					...
					break;
				case 6:
					String maleOwnerCity = InputUtil.acceptCityOfMaleOwner(scanner);
					String femaleOwnerCity = InputUtil.acceptCityOfFemaleOwner(scanner);
					ownerDTOList = ownerService.findAllOwnersWithCriteraOnGenderAndCity(maleOwnerCity, femaleOwnerCity);
					System.out.println("There are " + ownerDTOList.size() + " owners where male owner is from "
							+ maleOwnerCity + " and female owner is from " + femaleOwnerCity);
					ownerDTOList.forEach(System.out::println);
					break;
				case 7:
					ownerDTOList = ownerService.findAllOwnersWithCriteraOnNotEqualOwnerPetGender();
					System.out.println("There are " + ownerDTOList.size()
							+ " owners where gender of owner and pet are different.");
					ownerDTOList.forEach(System.out::println);
					break;
				default:
