...
public class InputUtil {
	...
	public static int acceptMenuOption(Scanner scanner) {
		...
		System.out.println("Press 6 to fetch all owners by sorting.");
		System.out.println("Press 7 to fetch all owners by pagination.");
		System.out.println("Press 8 to fetch all owners by pagination and sorting.");
	...
	public static boolean wantToSortByDescending(Scanner scanner) {
		System.out.println("Press Y to sort by descending order and N to exit.");
		char choice = scanner.next().toUpperCase().charAt(0);
		return 'Y' == choice;
	}

	public static int acceptRecordsPerPage(Scanner scanner) {
		System.out.println("Enter number of records per page:");
		return scanner.nextInt();
	}
