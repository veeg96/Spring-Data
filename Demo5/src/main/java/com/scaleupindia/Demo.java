		...
		try (Scanner scanner = new Scanner(System.in)) {
			do {
				...
				case 6:
					...
					System.out.println("There are " + ownerDTOList.size() + " owners sorted by " + sortingParameter);
					ownerDTOList.forEach(System.out::println);
					break;
				case 7:
					int itemsPerPage = InputUtil.acceptRecordsPerPage(scanner);
					long totalNumberOfOwners = ownerService.findTotalNumberOfOwners();
					int totalPages = (int) (totalNumberOfOwners / itemsPerPage);
					for (int pageNumber = 0; pageNumber < totalPages; pageNumber++) {
						ownerDTOList = ownerService.findAllPaginatedOwners(pageNumber, itemsPerPage);
						System.out.println("There are " + ownerDTOList.size() + " owners on page number " + pageNumber);
						ownerDTOList.forEach(System.out::println);
					}
					break;
				case 8:
					...
					itemsPerPage = InputUtil.acceptRecordsPerPage(scanner);
					totalNumberOfOwners = ownerService.findTotalNumberOfOwners();
					totalPages = (int) (totalNumberOfOwners / itemsPerPage);
					for (int pageNumber = 0; pageNumber < totalPages; pageNumber++) {
						ownerDTOList = ownerService.findAllPaginatedAndSortedOwners(pageNumber, itemsPerPage,
								sortingParameter, sortDescending);
						System.out.println("There are " + ownerDTOList.size() + " owners on page number " + pageNumber
								+ " sorted by " + sortingParameter);
						ownerDTOList.forEach(System.out::println);
					}
					break;
				default:
